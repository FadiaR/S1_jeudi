...
puts "Il y a #{handle.length} handle"


#Combien d'handle contiennent un numéro ?
counter = 0
handle.each do |element|
 if element.count("0-9") > 0
   counter = counter +1
   end
end 
...
counter = 0
handle.each do |nom|
  if nom[1].count("A-Z") >0 
    counter = counter + 1   
  end
end
puts "Il y a #{counter} handle qui commence par une majuscule"
...
puts "Il y a #{counter} qui contiennent une majuscule"
...
counter =0
handle.each do |nom|
    counter = nom.count("_") + counter 
 end

puts "Il y a #{counter} qui contiennent le signe _"
